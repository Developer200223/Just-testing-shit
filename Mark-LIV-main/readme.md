...
## 🧠 Expanded Mission Brief: an autonomous desktop JARVIS

The next generation of MARK LIV should behave like a general-purpose operating-system agent: a persistent digital worker that can observe, reason, act, report, and recover across desktop applications, games, files, and external messaging channels. The goal is not a cheat engine or a bypass of account safeguards; it is a robust, human-supervised automation system that can use vision, accessibility APIs, and structured automation to complete useful tasks and report status back to the user.

### Core mission

MARK LIV should be able to:

* interpret the desktop and game world through a vision stack at 720p and fallback to OCR, UI detection, and screenshot comparison when needed;
* control keyboard, mouse, windows, browser tabs, apps, and game clients across Steam, Epic Games, Roblox, Xbox app, Rockstar Games Launcher, and other desktop launchers;
* navigate interfaces, read dashboards, interact with menus, and manage user requests from a single orchestration layer;
* operate in a safe, auditable mode where every action is logged, reversible when possible, and optionally requires confirmation before destructive or account-impacting tasks;
* generate reports and send status updates through WhatsApp, Snapchat, Discord, email, Telegram, or other configured messaging channels;
* record gameplay, screenshots, and session logs for debugging, QA, and human review;
* code, create files, troubleshoot software, build small tools, and run local dev workflows on the same machine;
* build, test, and debug software projects, helper scripts, and automation routines while maintaining a memory of user preferences and active work;
* handle vehicle and game-related automation tasks, including sim/creator workflows, local app automation, and UI-based testing;
* provide a single user-facing interface where the operator can issue commands such as "build a car in-game," "record my session," "test the app," or "send a status update by Snapchat."

### Design expectations

The system should be designed as a layered architecture:

1. Perception layer
   * screen capture at 720p as the default mode;
   * OCR and UI element detection for buttons, forms, text, and game menus;
   * object and scene detection for games and desktop workflows;
   * screenshot diffing for state detection and action verification;
   * optional higher-resolution capture only when a task requires it.

2. Action layer
   * keyboard and mouse automation using OS-level input APIs;
   * window focus management, app launching, and user-session control;
   * browser automation for web tasks and UI-driven flows;
   * game launcher and desktop app selectors for Steam, Epic, Roblox, Xbox, Rockstar, and similar hubs;
   * a task scheduler for repeated work and monitoring.

3. Reasoning layer
   * a planning loop that can break larger tasks into steps;
   * memory of goals, previous outcomes, and user preferences;
   * tool-call orchestration with a verification pass after each action;
   * confidence checks before irreversible or high-risk steps.

4. Reporting layer
   * status snapshots with what happened, what is happening, and what is next;
   * summary messages for WhatsApp, Snapchat, Telegram, Discord, and email;
   * screenshot and recording attachments for visual reports;
   * active session telemetry such as CPU, RAM, GPU, stance of the task, and health of the environment.

5. Safety and policy layer
   * do not bypass platform anti-cheat or account controls;
   * honor eula, platform rules, and OS permissions;
   * require human confirmation for irreversible tasks or actions affecting payment, account security, or machine state;
   * operate in a "safe automation" mode that never tries to cheat, evade, or manipulate competitive systems.

### Hardware target

The system should be optimized for a practical mid-range workstation profile, not a luxury server rig:

* CPU: AMD Ryzen 5 3600
* GPU: NVIDIA GTX 1660 Ti 6GB
* memory target: under 6GB RAM for the active agent loop under normal operation; aggressive caching and frame-limited processing should be used to stay under this ceiling;
* network target: around 200 Mbps, with adaptive streaming and smaller image payloads when the connection is constrained;
* default vision mode: 720p capture and inference; only increase resolution if a task genuinely requires it.

### Software stack targets

The autonomous desktop agent should integrate the following categories of capabilities under one control loop:

* screen capture and visual state recognition;
* OCR and UI recognition for menus and text-heavy interfaces;
* desktop automation, click-path generation, and keyboard/mouse orchestration;
* game launcher and application integration;
* browser and web automation;
* task execution, file editing, code generation, and script automation;
* remote report delivery across messaging services;
* telemetry and session logging for debugging and reliability.

### Game and desktop compatibility scope

The active scope should include support patterns for:

* Steam games and launcher flows;
* Epic Games launcher and integrations;
* Roblox and browser-based launchers;
* Xbox app and game client flows;
* Rockstar Games Launcher and related launch patterns;
* generic Windows desktop UIs and client apps;
* browser-based games, lobby flows, and game launchers as a fallback control layer.

This is a design target, not a promise that every title can be perfectly automated without user-specific tuning. A strong system should support a broad catalog, but every game will require a tailored strategy for layout detection, control mapping, and safe interaction windows.

### Messaging and reporting requirements

The system should support a command channel where the user can request actions through a messaging app such as Snapchat or WhatsApp, and receive back:
...
Delivery should be compact and readable, with optional attachments for visual proof or logs.

### Development principles

* Keep the agent explainable and observable.
* Prefer deterministic automation over opaque guesses.
* Use vision as a fallback and confirmation layer instead of a total replacement for inputs.
* Treat every game and app as a unique automation target with user consent and safety constraints.
* Log every action and keep a reversible task ledger when possible.
* Optimize for robustness, not raw flash.

### Outcome

The final result should be a living, multi-skill desktop intelligence: a digital operator that can code, create, navigate apps, manage systems, play games within safe policy bounds, collect visual state, run automation, issue reports, and keep the user informed through their preferred messaging service.
