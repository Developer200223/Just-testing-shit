...
def _send_snapchat(receiver: str, message: str) -> str:
    """Open Snapchat Web so the user can authenticate before messaging.

    This project cannot create or authenticate a Snapchat account automatically.
    The user completes sign-in in the browser, then retries the message.
    """
    web_url = "https://web.snapchat.com/"
    login_url = "https://accounts.snapchat.com/accounts/login"

    if not _open_browser_url(web_url):
        return (
            "Could not open Snapchat Web. Please sign in at "
            f"{web_url} or log in at {login_url}."
        )

    return (
        f"Please sign in to Snapchat Web, then retry sending to {receiver}. "
        f"I opened {web_url}. I will not handle your password or verification code. "
        f"The message is ready to retry: '{message}'."
    )
...
_PLATFORM_MAP = [
    ...
    ({"snapchat", "snap", "snap chat"},      _send_snapchat),
    ({"signal"},                               _send_signal),
...
TOOL = {
    "name": "send_message",
    "description": "Sends a text message via WhatsApp, Telegram, Snapchat, or other messaging platform.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "receiver": {
            ...
            "platform": {
                "type": "STRING",
                "description": "Platform: WhatsApp, Telegram, Snapchat, etc. For Snapchat, the user must create and log into a Snapchat account first."
            }
        },
        "required": [
            "receiver",
